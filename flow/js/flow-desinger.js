 ...
 * @param ev
 */
window.onFlowDialogObjectDoubleClicked=function(ev, fromContext){
    ...
    if(window.showFlowNodeDetailInfo){
        ...
        if(ev.diagram&&ev.diagram.model){
            node.data.templateId=ev.diagram.model.templateId;
            nodeDataArray=ev.diagram.model.nodeDataArray;
            linkDataArray=ev.diagram.model.linkDataArray;
        }
        window.showFlowNodeDetailInfo(node.data, fromContext, nodeDataArray, linkDataArray);
        return;
    }
};
function FlowDesigner(diagramDiv) {
        ...
        return G(go.Adornment, "Vertical",
            makeMenuItem("编辑",
                function(e, obj) { // OBJ is this Button
                    ...
                    window.onFlowDialogObjectDoubleClicked({subject:{part:part}}, true);
                }),
            makeMenuItem("删除",
    ...
    function makeLinkTemplate(){
        return G(go.Link, // the whole link panel
            ...
            G(go.Shape, // 线条
                { stroke: "black",isPanelMain: true,strokeWidth: 2 }, new go.Binding("stroke")),
            G(go.Shape, // 箭头
    ...
    function onDiagramModified(e){
        if(window.onFlowDiagramModified){
            window.onFlowDiagramModified(_designer.isModified);
        }
    };
